...
export default function ReadingSection({ onBack }: ReadingSectionProps) {
  const { getDayHabits, updateLectura } = useHabits()
  const today = formatDate(new Date())
  const dayHabits = getDayHabits(today)
  const [completed, setCompleted] = useState(dayHabits.lectura)

  const handleToggle = () => {
    const newValue = !completed
    setCompleted(newValue)
    updateLectura(today, newValue)
  }
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-8">
          <div className="text-center">
            ...
            <p className="text-gray-600 dark:text-gray-400 mb-8">
              Marca si completaste tu sesión de lectura
            </p>
            <button
